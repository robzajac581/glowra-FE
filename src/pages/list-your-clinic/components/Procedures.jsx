...
const Procedures = ({ initialProcedures, providers, onContinue, onSkip, onBack, isEditMode = false }) => {
  ...
  const parsedProcedures = React.useMemo(() => {
  ...
  const handleBulkAdd = () => {
    ...
    const newProcedures = parsedProcedures.map(name => ({
      procedureName: name,
      category: '',
      priceMin: '',
      priceMax: '',
    ...
    const uniqueNewProcedures = newProcedures.filter(p => 
      !existingNames.has(p.procedureName.toLowerCase())
    );
    
    setProcedures([...procedures, ...uniqueNewProcedures]);
    setBulkInput('');
